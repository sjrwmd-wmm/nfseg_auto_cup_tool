...
class MFFloatArray:
    ...
    def writeFloatArrayAsTable(self, array_layer):
        """ Write the data to an ASCII file with two columns: cell sequence number, and cell value. """
        outputFileName = "dh_lyr{0}".format(array_layer)
        outputFile = open(outputFileName + '_tableFormat.csv', 'w')
...
def read_spec_file():
    f = open('array_reader.spc', 'r') # !!! PMB !!! Send name as input
    lines = f.readlines()
    f.close()    
    ...
    return (layers, rows, columns, valuesPerLine, fieldWidth, fieldFormat, head_noflow, head_dry)

def read_array_file_names():
    ...
    """
    array_names = []
    f = open('sim_head_arrays_file_names.asc', 'r') # !!! PMB !!! Send name as input
...
#def main():
    ...
    minValueCell = (0,0,0)
    layers, rows, columns, valuesPerLine, fieldWidth, fieldFormat, head_noflow, head_dry = read_spec_file()
    myGrid = ModelGrid((layers, rows, columns))    

    array_dict = {}
    #book = shelve.open('test.shelf')
    array_file_name_tuples = read_array_file_names()
    ...
    #book.close()
    print("    Finished reading arrays")
    return array_dict

def main(logfile): # !!! PMB !!! Search out the input filenames throughout
    ...
    """
    array_dict = read_arrays()
    print("    All done processing arrays")
